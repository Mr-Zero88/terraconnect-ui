...
export type Component<T = {}, S = T & { [data: `data-${string}`]: any }> = (this: HTMLComponent<T>, props: State<S | { children: Array<HTMLComponent<unknown>> }> | S | { children: Array<HTMLComponent<unknown>> }) => stateify<HTMLComponent<unknown> | Array<HTMLComponent<unknown>>>;
export type ComponentFN<T = {}> = (this: HTMLComponent<T>, props: State<T & { children: Array<HTMLComponent<unknown>> }>) => stateify<HTMLComponent<unknown> | Array<HTMLComponent<unknown>>>;
export type HTMLComponent<T> = HTMLElement & { component: string | Component<any>, props: State<HTMLComponentProps<T>> };
...
export function applyPropsAsAtributes<T>(element: HTMLComponent<T>, props: props<T>) {
  ...
  const setAttribute = (key: string | null, value: unknown) => {
    ...
    element?.setAttribute(key.replaceAll('on', 'on-'), data);
  }
  Object.entries<State<unknown>>(attributes).forEach(([key, state]) => setAttribute(key, state[Value]));
  attributes[ChildModified].on((newValue, key) => setAttribute((typeof key == "string") ? key : null, newValue));
...
export function appendChildren<T>(element: HTMLComponent<T>, children: State<Array<HTMLComponent<T> | Text | null>>) {
...
export function formatChildren<T>(children: children): State<Array<HTMLComponent<T> | Text | null>> {
  if (children != null) {
    if (isState(children)) {
  ...
  return (children as State<Array<HTMLComponent<T> | string | null>>).map[Value]<Array<HTMLComponent<T> | Text | null>>(({ [Value]: child }) => child == null ? null : child instanceof Node ? child : document.createTextNode(child.toString()));
}